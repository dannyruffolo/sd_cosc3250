...
syscall user_printf(const char *format, ...) {
    ...
    // Use SCARG to get the arguments for user_putc
    // Check if the first argument is 0 - if it is then assign it to temp for descrp, and assign other arg to temp for character
    // If the first argument is not 0, check if the second arg is - if it is assign it to descrp temp and other to character temp
    // Once you have the assigned temp variables, pass descrp temp in for descrp and character temp for character in order
    // This then combats the issue of the order in which arguments are passed into user_putc v. the doprnt function
